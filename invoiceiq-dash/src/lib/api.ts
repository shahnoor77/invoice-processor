...
export const apiToggleEmailConfig = () => req<{ is_active: boolean; message: string }>('PATCH', '/settings/email/toggle')
export const apiPollNow = () => req('POST', '/settings/email/poll-now')
export const apiDeleteInvoice = (id: string) => req('DELETE', `/invoices/${id}`)
export const apiUpdateInvoice = (id: string, data: unknown) => req<RealInvoice>('PATCH', `/invoices/${id}`, data)
